...
import {useFetchTourGuideDetails  } from "../../api/endPints";
import { useParams } from "react-router-dom";

const Description = () => {
  const { id } = useParams();

  const { data: tour, isLoading, error } = useFetchTourGuideDetails (id);
  ...
  return (
    <div className="max-w-8xl mx-auto bg-white rounded-lg p-7 flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-1/2">
        <img
          src={`http://localhost:5200${tour.profilePictureUrl}`}
          alt={tour?.firstname || "TourGuide"}
        ...
        <div className="flex gap-10 items-center mt-4 text-sm text-gray-700 px-1">
          ...
          <div className="flex items-center gap-1">
            <span>Entry Fee: {tour.hourlyRate }$</span>
          </div>
          <div className="flex items-center gap-1">
            <span>
              Time: {tour.openTime || "09:00"} - {tour.closeTime || "17:00"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-around text-left mt-4 md:mt-0">
        <div>
          ...
          <p className="text-gray-800 mb-2">{tour.bio}</p>
        </div>
