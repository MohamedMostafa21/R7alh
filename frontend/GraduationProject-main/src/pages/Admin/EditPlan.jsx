...
import { useFetchPlansDetails, useUpdatePlan } from "../../api/endPints";

export default function EditPlaceForm() {
  const { id } = useParams();
  const { data: plan, isLoading, error } = useFetchPlansDetails(id);
  const updatePlaceMutation = useUpdatePlan();
  ...
  useEffect(() => {
    if (plan) {
      setFormData({
        name: plan.name || "",
        tourGuideName: plan.tourGuideName || "",
      ...
      if (plan.thumbnailUrl) {
        const fullUrl = `http://localhost:5200${plan.thumbnailUrl}`;
        setThumbnail({
          preview: fullUrl,
      ...
      if (plan.planPlaces && plan.planPlaces.length > 0) {
        const loaded = plan.planPlaces.map((url, i) => ({
          id: `existing-${i}`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const existingplanPlaces = images
        .filter((img) => img.isExisting && img.url)
        .map((img) => img.url);

      existingplanPlaces.forEach((url) => {
        fd.append("ExistingplanPlaces", url);
      });

      console.log(
        `Sending ${newImages.length} new images and keeping ${existingplanPlaces.length} existing images`
      );

      await updatePlaceMutation.mutateAsync({ id, data: fd });
      alert("Place updated successfully!");
    } catch (error) {
      console.error("Update error:", error);
      alert(
        "Error while updating place: " + (error.message || "Unknown error")
      );
    } finally {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 mb-4">Error loading plan details</div>
          <Link
            to="/Admin"
    ...
    <div className="w-full min-h-screen bg-slate-100">
      <div className="bg-slate-200 py-4 px-6 flex justify-between items-center shadow-lg">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Edit Plan</h1>
        </div>
        <Link
          to="/Admin"
            ...
            {images.map((img) => (
              <div key={img.id} className="relative group">
                ...
                <button
                  ...
                  className="absolute -top-2 -right-2 bg-red-600 text-white p-1 rounded-full hover:bg-red-700 transition-colors opadays-0 group-hover:opadays-100"
                  type="button"
                >
                  <X className="w-3 h-3" />
                </button>
