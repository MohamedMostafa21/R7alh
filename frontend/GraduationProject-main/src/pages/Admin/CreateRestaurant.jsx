...
import { useCreateRestaurant } from "../../api/endPints";
...
export default function AddRestaurantForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createRestaurantMutation.mutateAsync(fd);
      alert("Restaurant created successfully!");
    } catch (error) {
      console.error("Create error:", error);
      alert("Error while creating restaurant: " + (error.message || "Unknown error"));
    } finally {
    ...
    <div className="w-full min-h-screen bg-slate-100">
     <div className="bg-slate-200 py-4 px-6 flex justify-around items-center shadow-lg">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Add New Restaurant</h1>
        </div>
         <div className="">
            ...
            {images.map((img) => (
              <div key={img.id} className="relative group">
                ...
                <button
                  ...
                  className="absolute -top-2 -right-2 bg-red-600 text-white p-1 rounded-full hover:bg-red-700 opacity-0 group-hover:opacity-100"
                  type="button"
                >
                  <X className="w-3 h-3" />
                </button>
