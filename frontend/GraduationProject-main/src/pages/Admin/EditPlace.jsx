...
import { useFetchPlaceDetails, useUpdatePlace } from "../../api/endPints";
  ...
  useEffect(() => {
    if (place) {
      setFormData({
        ...
        averagePrice: place.averagePrice?.toString() || "",
      });

      if (place.thumbnailUrl) {
        const fullUrl = `http://localhost:5200${place.thumbnailUrl}`;
        setThumbnail({
          preview: fullUrl,
      ...
      if (place.imageUrls && place.imageUrls.length > 0) {
        const loaded = place.imageUrls.map((url, i) => ({
          id: `existing-${i}`,
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 mb-4">Error loading place details</div>
          <Link
            to="/Admin"
    ...
    <div className="w-full min-h-screen bg-slate-100">
      <div className="bg-slate-200 py-4 px-6 flex justify-between items-center shadow-lg">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Edit Place</h1>
        </div>
        <Link
          to="/Admin"
