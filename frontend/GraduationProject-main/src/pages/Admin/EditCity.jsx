...
import { useFetchCitiesDetails, useUpdateCity } from "../../api/endPints";

export default function EditCityForm() {
  const { id } = useParams();
  const { data: city, isLoading, error } = useFetchCitiesDetails(id);
  const updatePlaceMutation = useUpdateCity();
  ...
  useEffect(() => {
    if (city) {
      ...
      if (city.thumbnailUrl) {
        const fullUrl = `http://localhost:5200${city.thumbnailUrl}`;
        setThumbnail({
          preview: fullUrl,
          url: city.thumbnailUrl,
          isExisting: true,
        });
      }

      if (city.imageUrls && city.imageUrls.length > 0) {
        const loaded = city.imageUrls.map((url, i) => ({
          id: `existing-${i}`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("Error while updating city: " + (error.message || "Unknown error"));
    } finally {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 mb-4">Error loading city details</div>
          <Link
            to="/Admin"
    ...
    <div className="w-full min-h-screen bg-slate-100">
      <div className="bg-slate-200 py-4 px-6 flex justify-between items-center shadow-lg">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Edit City</h1>
        </div>
        <Link
          to="/Admin"
