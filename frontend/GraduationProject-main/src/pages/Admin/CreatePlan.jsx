...
import { useCreatePlan } from "../../api/endPints";
...
export default function AddPlanForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createPlanMutation.mutateAsync(fd);
      alert("Plan added successfully!");
    } catch (error) {
      console.error("Create error:", error);
      alert("Error while adding plan: " + (error.message || "Unknown error"));
    } finally {
    ...
    <div className="w-full min-h-screen bg-slate-100">
     <div className="bg-slate-200 py-4 px-6 flex justify-around items-center shadow-lg">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Add New Plan</h1>
        </div>
         <div className="">
      ...
      <div className="max-w-6xl mx-auto bg-white p-6 mt-6 shadow rounded space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input name="tourGuideName" value={formData.tourGuideName} onChange={handleChange} placeholder="Tour Guide Name" className="p-2 border rounded focus:ring-2 focus:ring-blue-500" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <input name="days" value={formData.days} onChange={handleChange} placeholder="Days" className="p-2 border rounded focus:ring-2 focus:ring-blue-500" />
          <input name="price" value={formData.price} onChange={handleChange} placeholder="Price" className="p-2 border rounded focus:ring-2 focus:ring-blue-500" />
        </div>
