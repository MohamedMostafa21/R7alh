...
import { useFetchHotelsDetails, useUpdateHotel } from "../../api/endPints";

export default function EdithotelForm() {
  const { id } = useParams();
  const { data: hotel, isLoading, error } = useFetchHotelsDetails(id);
  ...
  useEffect(() => {
    if (hotel) {
      ...
      if (hotel.thumbnailUrl) {
        const fullUrl = `http://localhost:5200${hotel.thumbnailUrl}`;
        setThumbnail({
          preview: fullUrl,
      ...
      if (hotel.imageUrls && hotel.imageUrls.length > 0) {
        const loaded = hotel.imageUrls.map((url, i) => ({
          id: `existing-${i}`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updatehotelMutation.mutateAsync({ id, data: fd });
      alert("hotel updated successfully!");
    } catch (error) {
      console.error("Update error:", error);
      alert(
        "Error while updating hotel: " + (error.message || "Unknown error")
      );
    } finally {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 mb-4">Error loading hotel details</div>
          <Link
            to="/Admin"
    ...
    <div className="w-full min-h-screen bg-slate-100">
      <div className="bg-slate-200 py-4 px-6 flex justify-between items-center shadow-lg">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Edit Hotel</h1>
        </div>
        <Link
          to="/Admin"
      ...
      <div className="max-w-6xl mx-auto bg-white p-6 mt-6 shadow rounded space-y-4">
        ...
        <div>
          <label className="block text-left font-medium mt-6 mb-6">Images</label>
